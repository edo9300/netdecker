...
bool SaveDeck(const path_string& name, std::vector<int> mainlist, std::vector<int> extralist) {
...
int main(int argc, char* argv[]) {
	...
	for(int i = 0; i < decks.size(); i++) {
                SaveDeck(I18N("player ")+ convert_string(players[i]) + I18N(" ") + tostring(i)+ I18N(".ydk"), decks[i].main_deck, decks[i].extra_deck);
	}
